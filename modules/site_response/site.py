...
def _load_published_curves(fpath):
...
def known_published_curves(fpath) -> list[dict]:
    """List of published curves in the database."""
    _load_published_curves(fpath)
...
class NonlinearProperty:
    ...
    @classmethod
    def from_published(cls, name, param, fpath=None):
        assert param in cls.PARAMS
        _load_published_curves(fpath)

        if name not in PUBLISHED_CURVES:
            raise KeyError(f"'{name}' not found in PUBLISHED_CURVES. Available keys: {PUBLISHED_CURVES.keys()}")

        if param not in PUBLISHED_CURVES[name]:
            raise KeyError(f"'{param}' not found in '{name}'. Available keys: {PUBLISHED_CURVES[name].keys()}")
...
class SoilType:
    ...
    def __init__(
        ...
        **kwargs,
        ...
        # Asignación directa de parámetros definidos en la clase
        self.name = name
        self._unit_wt = unit_wt
        self.mod_reduc = mod_reduc
        self.damping = damping

        # Asignación dinámica de atributos adicionales
        self._set_attributes(**kwargs)

    def _set_attributes(self, **kwargs):
        """
        Establece dinámicamente los atributos en la instancia
        solo si existen como atributos de la clase.
        """
    ...
    def from_published(
        ...
        return cls(
            name,
            unit_wt=unit_wt,
            mod_reduc=NonlinearProperty.from_published(model, "mod_reduc", fpath),
            damping=NonlinearProperty.from_published(model_damping, "damping", fpath),
        )
